...
function Blogs() {
  ...
  const allTags = Array.from(new Set(blogs.flatMap(blog => Array.isArray(blog.tags) ? blog.tags : typeof blog.tags === 'string' ? [blog.tags] : [])));
  ...
  if (activeTag) {
    filteredBlogs = filteredBlogs.filter(blog => (Array.isArray(blog.tags) ? blog.tags : typeof blog.tags === 'string' ? [blog.tags] : []).includes(activeTag));
  }
  if (showPopular) {
  ...
  if (searchQuery) {
    filteredBlogs = filteredBlogs.filter(blog =>
      ...
      (Array.isArray(blog.tags) ? blog.tags : typeof blog.tags === 'string' ? [blog.tags] : []).some(tag => tag.toLowerCase().includes(searchQuery.toLowerCase()))
    );
  }
  ...
  return (
    <Box sx={{ position: 'relative', minHeight: '100vh', bgcolor: alpha(theme.palette.primary.light, 0.1), pt: 10, pb: 8 }}>
          ...
          <Grid item xs={12} md={8}>
            <Box sx={{ mb: 4, textAlign: 'center', zIndex: 1 }}>
              <Typography variant="h2" component="h1" sx={{ fontWeight: 'bold', color: theme.palette.primary.main, mb: 2, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 2 }}>
                <PetsIcon sx={{ fontSize: 40 }} /> Pet Care Blog <PetsIcon sx={{ fontSize: 40 }} />
              </Typography>
            ...
            <Stack spacing={4}>
              {filteredBlogs.map((blog) => {
                let tags = Array.isArray(blog.tags) ? blog.tags : typeof blog.tags === 'string' ? [blog.tags] : [];
                return (
                  <motion.div
                    key={blog._id}
                    initial="offscreen"
                    ...
                    style={{ width: '100%' }}
                  >
                    <Card sx={{ borderRadius: 4, boxShadow: theme.shadows[4], transition: 'all 0.3s', '&:hover': { boxShadow: theme.shadows[8], transform: 'translateY(-2px)' }, overflow: 'hidden', position: 'relative', background: 'linear-gradient(135deg, #fff8e1 0%, #ffecb3 100%)' }}>
                      <Box
                        sx={{
                          width: '100%',
                          height: 340,
                          background: 'linear-gradient(135deg, #ffe5b4 0%, #fffbe7 100%)',
                          display: 'flex',
                          alignItems: 'center',
                      ...
                      <CardContent>
                        ...
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mt: 2 }}>
                          ...
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, cursor: 'pointer', '&:hover': { animation: `${pulse} 0.5s ease-in-out` } }}>
      ...
      {/* Floating Action Button for New Blog */}
      <Fab
        color="primary"
        aria-label="add"
        sx={{
