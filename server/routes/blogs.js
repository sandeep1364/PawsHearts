...
router.post('/', auth, upload.single('image'), async (req, res) => {
  try {
    ...
    let tags = req.body.tags; // Get the tags from the request body

    // Attempt to parse tags if they are a string (as sent by client FormData)
    if (typeof tags === 'string') {
      try {
        tags = JSON.parse(tags);
      } catch (parseError) {
        console.error('Error parsing tags JSON:', parseError);
        // If parsing fails, treat tags as an empty array
        tags = [];
      }
    }
    ...
    // --- Add validation to check if at least one tag is provided ---
    if (tagsArray.length === 0) {
      if (req.file) {
        // Clean up uploaded file if validation fails
        fs.unlinkSync(req.file.path);
      }
      return res.status(400).json({ message: 'At least one topic tag is required.' });
    }
    // -----------------------------------------------------------------
    ...
    const blog = new Blog({
      ...
      tags: tagsArray // Use the parsed/verified array
    });
...
router.get('/', async (req, res) => {
  try {
    res.json({ message: 'Blogs routes working' });
  } catch (error) {
    res.status(500).json({ message: 'Error in blogs routes' });
  }
});
